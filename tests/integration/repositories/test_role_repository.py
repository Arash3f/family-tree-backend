...
async def test_create_and_get_role(uow: UnitOfWork):
    perm = await uow.permissions.create(Permission(id=None, name="can_manage_users"))
...
async def test_get_list_by_filter_with_permission_id(uow: UnitOfWork):
    ...
    await uow.roles.create(Role(id=None, name="role2", permission_ids=[perm2.safe_id]))
...
async def test_delete_role(uow: UnitOfWork):
    role = await uow.roles.create(Role(id=None, name="to_delete", permission_ids=[]))
