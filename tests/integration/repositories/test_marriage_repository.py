...
async def test_create_and_get_marriage(uow: TreeUnitOfWork):
    async with uow:
        ...
        assert fetched.spouse_a_id == new_marriage.spouse_a_id
        assert fetched.spouse_b_id == new_marriage.spouse_b_id
...
async def test_update_marriage(uow: TreeUnitOfWork):
    async with uow:
        ...
        marriage.spouse_a_id = husband_2.safe_id
        marriage.spouse_b_id = wife_2.safe_id
        marriage.married_at = date(2001, 1, 1)

        updated = await uow.marriages.update(marriage=marriage)

        assert updated.spouse_b_id == wife_2.safe_id
        assert updated.spouse_a_id == husband_2.safe_id
