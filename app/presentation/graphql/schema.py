...
class Query:
    ...
    @strawberry.field(description="Get person by id (REST: GET /persons/{id})")
    async def person(self, info: strawberry.Info, person_id: UUID) -> PersonType:
        return await person_resolvers.resolve_person(info, person_id)

    @strawberry.field(description="Filtered person list (REST: POST /persons/list)")
    async def persons(
        self,
        info: strawberry.Info,
        data: PersonListInput | None = None,
    ) -> PersonPage:
        return await person_resolvers.resolve_persons(info, data)

    @strawberry.field(
        description="Closest relationship path (REST: GET /persons/{from}/relation/{to})"
    )
    async def closest_relationship(
        ...
        return await person_resolvers.resolve_closest_relationship(
            info, from_person_id, to_person_id
        )
    ...
    @strawberry.field(description="Get marriage by id (REST: GET /marriages/{id})")
    async def marriage(self, info: strawberry.Info, marriage_id: UUID) -> MarriageType:
        return await marriage_resolvers.resolve_marriage(info, marriage_id)

    @strawberry.field(description="Filtered marriage list (REST: POST /marriages/list)")
    async def marriages(
        self,
        info: strawberry.Info,
...
class Mutation:
    ...
    @strawberry.mutation(description="Create person (REST: POST /persons/)")
    async def create_person(
        self,
        info: strawberry.Info,
        data: PersonCreateInput,
    ) -> PersonType:
        return await person_resolvers.resolve_create_person(info, data)

    @strawberry.mutation(description="Update person (REST: PUT /persons/)")
    async def update_person(
        self,
        info: strawberry.Info,
        data: PersonUpdateInput,
    ) -> PersonType:
        return await person_resolvers.resolve_update_person(info, data)

    @strawberry.mutation(description="Delete person (REST: DELETE /persons/{id})")
    async def delete_person(
        self,
        info: strawberry.Info,
        person_id: UUID,
    ) -> ResultType:
        return await person_resolvers.resolve_delete_person(info, person_id)
    ...
    @strawberry.mutation(description="Create marriage (REST: POST /marriages/)")
    async def create_marriage(
        self,
        info: strawberry.Info,
        data: MarriageCreateInput,
    ) -> MarriageType:
        return await marriage_resolvers.resolve_create_marriage(info, data)

    @strawberry.mutation(description="Update marriage (REST: PUT /marriages/)")
    async def update_marriage(
        self,
        info: strawberry.Info,
        data: MarriageUpdateInput,
    ) -> MarriageType:
        return await marriage_resolvers.resolve_update_marriage(info, data)

    @strawberry.mutation(description="Delete marriage (REST: DELETE /marriages/)")
    async def delete_marriage(
        self,
        info: strawberry.Info,
        marriage_id: UUID,
    ) -> ResultType:
        return await marriage_resolvers.resolve_delete_marriage(info, marriage_id)

    @strawberry.mutation(description="Divorce (REST: POST /marriages/divorce)")
    async def divorce(
        self,
        info: strawberry.Info,
        data: DivorceInput,
