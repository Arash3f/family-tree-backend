...
class Query:
    ...
    @strawberry.field(
        description="List family trees for current user (REST: GET /family-trees)"
    )
    ...
    @strawberry.field(
        description="Get family tree by id (REST: GET /family-trees/{id})"
    )
    async def family_tree(self, info: strawberry.Info, tree_id: UUID) -> FamilyTreeType:
...
class Mutation:
    ...
    async def add_ticket_message(
        ...
        return await ticket_resolvers.resolve_add_ticket_message(info, ticket_id, data)
...
def _build_extensions() -> list[type[SchemaExtension] | Callable[[], SchemaExtension]]:
    """Bound the cost of a single GraphQL document.

    One query can fan out far more work than a REST call, so depth, alias and
    token limits are enforced before execution. Introspection stays available
    only where a developer needs the schema browser.
    """
    # Factories rather than instances: Strawberry builds a fresh extension per
    # request and deprecates sharing one instance across requests.
    extensions: list[type[SchemaExtension] | Callable[[], SchemaExtension]] = [
        AppExceptionExtension,
