...
_ERROR_TEXTS: dict[str, dict[str, str]] = {
    ...
    "fa": {
        ...
        "unreadable": (
            "این فایل به‌عنوان فایل اکسل باز نشد. آن را با قالب xlsx ذخیره کنید "
            "یا از فایل نمونه استفاده کنید."
        ),
        "missing_sheet": (
            "این فایل برگه‌ای با نام «{sheet}» ندارد. لطفاً فایل نمونه را دانلود "
            "کنید و اطلاعات را در برگه‌های همان فایل وارد کنید."
        ),
        "missing_columns": (
            "برگهٔ «{sheet}» این ستون‌ها را ندارد: {columns}. برای داشتن عنوان "
            "ستون‌های درست، فایل نمونه را دانلود کنید."
        ),
        "name_required": "ستون «{column}» خالی است؛ به همین دلیل این ردیف وارد نشد.",
        "duplicate_ref": (
            "کد «{ref}» پیش‌تر در ردیف {other} به کار رفته است. هر ردیف باید کد "
            "مخصوص به خود داشته باشد."
        ),
        "gender_invalid": (
            "مقدار «{value}» در ستون «{column}» معتبر نیست. یکی از این مقادیر را "
            "بنویسید: {options}."
        ),
        "gender_required": (
            "ستون «{column}» خالی است. یکی از این مقادیر را بنویسید: {options}."
        ),
        "rel_type_invalid": (
            "مقدار «{value}» در ستون «{column}» معتبر نیست. آن را خالی بگذارید یا "
            "یکی از این مقادیر را بنویسید: {options}."
        ),
        "date_invalid": (
            "مقدار «{value}» در ستون «{column}» تاریخ معتبری نیست. تاریخ را به شکل "
            "{hint} بنویسید یا سلول را خالی بگذارید."
        ),
        "married_at_required": (
            "ستون «{column}» خالی است. وارد کردن این تاریخ برای هر ازدواج الزامی است."
        ),
        "spouse_required": (
            "پر کردن هر دو ستون «{column_a}» و «{column_b}» الزامی است؛ به همین "
            "دلیل این ردیف وارد نشد."
        ),
        "unknown_parent": (
            "کد «{ref}» در ستون «{column}» در هیچ ردیفی از برگهٔ «{sheet}» وجود ندارد."
        ),
        "unknown_marriage": (
            "کد «{ref}» در ستون «{column}» در هیچ ردیفی از برگهٔ «{sheet}» وجود ندارد."
        ),
        "unknown_spouse": (
            "کد «{ref}» در ستون «{column}» نه در برگهٔ «{sheet}» وجود دارد و نه "
            "فردی با این کد در شجره‌نامه هست."
        ),
        "bio_parents_marriage": (
            "والدین بیولوژیک (تنی) باید همان دو همسری باشند که ازدواجشان در ستون "
            "«{column}» آمده است."
        ),
        "date_hint": "1398/02/03 (شمسی) یا 2019-04-23 (میلادی)",
        "namesake": (
            "نام، نام خانوادگی، جنسیت و تاریخ تولد این ردیف با ردیفِ کد «{ref}» در "
            "همین فایل یکسان است. هر دو به‌عنوان دو فرد جداگانه وارد می‌شوند."
        ),
        "ambiguous_identity": (
...
def _add_person_validations(ws, *, lang: str = "en") -> None:
    ...
    _add_list_validation(
        ...
        error=(
            "فقط «مرد» یا «زن» را انتخاب کنید."
            if locale == "fa"
            else "Use male or female"
        ),
    )
    ...
    _add_list_validation(
        ...
        error=(
            "فقط «بیولوژیک»، «فرزندخواندگی» یا «ناتنی» را انتخاب کنید."
            if locale == "fa"
...
_HOW_TO: dict[str, list[str]] = {
    ...
    "fa": [
        ...
        "و جنسیت در هر ردیف الزامی است.",
        "ستون «{code}» یک کد کوتاه دلخواه برای هر ردیف است، مثل P1 یا P2. این کد "
        "را خودتان تعیین می‌کنید و فقط وقتی لازم است که ردیف دیگری بخواهد به این "
        "فرد اشاره کند. اگر آن را خالی بگذارید، کدی به‌طور خودکار ساخته می‌شود.",
        "برای ثبت والد، کد او را در ستون «{parent_code}» بنویسید. ستون خاکستری "
        "«{parent_name}» در کنار آن هنگام خروجی گرفتن به‌طور خودکار پر می‌شود تا "
        "بتوانید فایل را بدون تطبیق دادن کدها بخوانید.",
        "برگهٔ «{marriages}» مخصوص ازدواج‌هاست: کد هر دو همسر و تاریخ ازدواج را "
        "وارد کنید.",
        "برای فرزندی که حاصل یک ازدواج است، کد آن ازدواج را در ستون "
        "«{marriage_code}» بنویسید. والدین بیولوژیک (تنی) او باید همان دو همسر "
        "باشند.",
        "تاریخ‌ها را می‌توانید به شکل {hint} بنویسید. اگر تاریخی را نمی‌دانید، "
        "آن را خالی بگذارید.",
        "ستون‌های خاکستری فقط برای راهنمایی شما پر می‌شوند و هنگام وارد کردن "
        "خوانده نمی‌شوند؛ پس تغییر دادن آن‌ها اثری ندارد.",
...
_PERSON_COLUMN_HELP: dict[str, dict[str, str]] = {
    ...
    "fa": {
        ...
        "ردیف دیگری به این فرد اشاره کند.",
        "name": "الزامی. نام فرد.",
        "family_name": "اختیاری.",
        "gender": "الزامی. «مرد» یا «زن».",
        "birth_date": "اختیاری. اگر تاریخ را نمی‌دانید، خالی بگذارید.",
        "death_date": "اختیاری. برای افرادی که در قید حیات هستند خالی بگذارید.",
        "birth_place": "اختیاری.",
        "death_place": "اختیاری.",
        "notes": "اختیاری. هر توضیحی که می‌خواهید ثبت شود.",
        "parent1_ref": "اختیاری. کد والد اول.",
        "parent1_name": "فقط برای نمایش. نام والد اول، برای راهنمایی شما.",
        "parent1_type": "اختیاری. «بیولوژیک» (پیش‌فرض)، «فرزندخواندگی» یا «ناتنی».",
        "parent2_ref": "اختیاری. کد والد دوم.",
        "parent2_name": "فقط برای نمایش. نام والد دوم، برای راهنمایی شما.",
...
_MARRIAGE_COLUMN_HELP: dict[str, dict[str, str]] = {
    ...
    "fa": {
        ...
        "ازدواج اشاره کند، لازم است.",
        "spouse_a_ref": "الزامی. کد همسر اول.",
        "spouse_a_name": "فقط برای نمایش. نام همسر اول، برای راهنمایی شما.",
        "spouse_b_ref": "الزامی. کد همسر دوم.",
...
_INSTRUCTION_LABELS: dict[str, dict[str, str]] = {
    ...
    "fa": {
        ...
        "export_title": "شجره‌نامه — خروجی اطلاعات",
        "how_to": "راهنمای پر کردن فایل",
